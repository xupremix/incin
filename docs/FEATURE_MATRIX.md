...
| `incin` | `metal` | preview | facade | `std` | no | `std`, `incin-backends/metal` | none | apple-silicon | Preview: the native Metal backend for Apple Silicon, covering the subset in docs/capabilities.md. Never enabled implicitly. |
| `incin` | `rocm` | preview | facade | `std` | no | `std`, `incin-backends/rocm` | none | rocm | Scaffolding: ROCm device identity, detection stub, and storage shells for AMD GPUs (issue #6). No HIP bindings yet, so no kernels and no allocation. |
...
| `incin` | `update-check` | stable | facade | `std` | no | `std`, `dep:ureq` | none | none | Lets `cargo incin doctor --check-updates` ask crates.io whether a newer incin exists. Off by default: it is the only feature that can reach the network, and no build should gain that ability without asking for it. |
...
| `incin-backends` | `rocm` | preview | backends | `std` | no | `std`, `incin-core/rocm` | none | rocm | Enables Incin's ROCm backend scaffolding for AMD GPUs (issue #6). Detection and storage metadata only: no HIP bindings are vendored, so no device memory is allocated and the capability table is empty. |
...
| `incin-core` | `rocm` | preview | core | none | no | none | none | rocm | Enables ROCm device marker metadata used by ROCm-capable backend crates (issue #6 scaffolding: identity and storage shells; no HIP bindings yet). |
