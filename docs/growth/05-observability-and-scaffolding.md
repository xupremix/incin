...
`Reporter` trait (`crates/kindle-telemetry/src/reporter.rs`) required
...
(`crates/kindle-telemetry/src/emitter.rs`) as the one-call constructor: it
...
**Task 05.2 (`cargo kindle watch`) — DONE, with one documented gap.** Added
the `watch` subcommand to `cargo-kindle.rs`: looks up `kindle-viz` on `PATH`
first (the `cargo install`-ed / published-user path), falling back to
`cargo run --quiet -p kindle-viz --` when not found (the in-workspace/dev
path, needing no separate install step). `--run-id`/`--run-dir` pass straight
through to `kindle-viz`'s own CLI parser unchanged. Verified for real:
...
vs `--file` transport selection" is only half-real. `kindle-telemetry` has a
write-side `SocketTransport` (`transport/socket.rs`), but `kindle-viz` has
**no socket reader at all** — only `FileTransportReader`
(`transport_reader.rs`). Adding `--socket` to `cargo kindle watch` today
...
`SocketTransportReader` for `kindle-viz` is real, additional scope, not
attempted this pass.

**Task 05.3 (`cargo kindle new <template>`) — PARTIAL.** Added the `new`
subcommand plus one embedded template (`mnist`,
`crates/kindle/src/bin/templates/mnist/{Cargo.toml,main.rs,README.md}.template`,
...
- **Path deps, not version deps.** The scaffold's `Cargo.toml` depends on
  `kindle`/`kindle-telemetry` via absolute path (computed from
  `cargo-kindle`'s own `env!("CARGO_MANIFEST_DIR")` at its compile time), not
  crates.io versions. `kindle-telemetry`'s `Cargo.toml` has `publish = false`
  — it can **never** be a version dependency, published or not, so this
  isn't a temporary shortcut for that one crate. `kindle`/`kindle-data` have
...
- **Synthetic random data, not real MNIST.** The scaffold trains on
  ...
  `kindle_data::vision::mnist::MnistDataset`, so `cargo build && cargo run`
  ...
  `crates/kindle/examples/mnist_training.rs` for swapping in genuine data.
...
Verified end-to-end for real (not just "compiles"): `cargo-kindle new mnist
...
the right `name`/`step`/`value`); `cargo kindle watch --run-id <that id>`
...
kindle-backends/cpu,kindle/cpu -- -D warnings` clean; `cargo test --workspace
--all-targets` (same features) all green, 0 failed across the whole
workspace; `cargo build --examples --workspace` clean; `cargo test -p
kindle-backends --no-default-features --features wgpu,std --lib` 97 passed;
