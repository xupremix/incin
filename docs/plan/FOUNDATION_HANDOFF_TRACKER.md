...
Next concrete action: remove the remaining standalone `load_safetensors_map` f32 storage helper and replace optimizer/checkpoint wire records with explicit heterogeneous state artifacts.
...
| STATE-003 | Serialization | safetensors/postcard load/save preserve supported state dtype | ModelExt safetensors/postcard paths now serialize owned snapshots; exact postcard and CPU round-trip pass | PARTIAL | P1 | buffer-role wire metadata; standalone hub loader still exposes f32 storage |
...
| Owned heterogeneous state artifact exists | PARTIAL | `nn/state.rs` validates exact `DTypeDescriptor`/`StorageEncoding` byte lengths; macro and sequential traversal collect/stage it; optimizer and hub compatibility maps remain separate blockers |
| Backend-neutral model persistence exists | PARTIAL | ModelExt safetensors/postcard and global checkpoint resharding use `StateSnapshot`; safetensors role metadata and explicit wire schema still need migration |
...
| --- | --- | --- |
| NEXT-A | Legacy state API removal | Model `StateDict` and serializer traits no longer expose homogeneous tensor maps; `nn/save.rs` resharding is snapshot-native. Remaining `load_safetensors_map`/hub API and optimizer dictionaries must be retired or explicitly separated from model state. |
