...
| STATE-002 | State | model state representation supports heterogeneous built-in dtypes safely | Owned `StatePath`/`StateValue`/`StateSnapshot` preserve exact descriptor, shape, role, and native bytes; typed leaves stage before commit | CONFIRMED FIXED | P0 | mixed f32/f16 extraction, exact-byte state validation, and legacy model-map removal |
| STATE-003 | Serialization | safetensors/postcard load/save preserve supported state dtype | ModelExt safetensors/postcard paths serialize owned snapshots; exact native-dtype round trips and explicit Q8_0 postcard/safetensors behavior pass | CONFIRMED FIXED | P1 | exact supported-dtype serializer tests and role metadata |
...
| Owned heterogeneous state artifact exists | CONFIRMED FIXED | `nn/state.rs` validates exact `DTypeDescriptor`/`StorageEncoding` byte lengths; macro and sequential traversal collect/stage it; mixed-dtype and strict-load tests pass. Optimizer persistence remains a separate blocker |
| Backend-neutral model persistence exists | CONFIRMED FIXED | ModelExt safetensors/postcard and global checkpoint resharding use `StateSnapshot`; safetensors role metadata, postcard wire records, and explicit checkpoint dtype records preserve semantic storage information |
