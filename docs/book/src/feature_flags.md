...
Each feature's declaration, tier, owner, prerequisites, and hardware boundary
are maintained in the owning crate's `Cargo.toml`. The inventory below is
generated by `cargo xtask docs`; do not edit it directly.
...
The default facade build is `std` plus `cpu`. Stable features are part of the
0.1 compatibility promise. Preview features are opt-in and can change without
a migration path. Test-only, hardware-only, and internal features are not
product configuration promises.

There are no rejected stable feature combinations in 0.1. The MSRV job checks
every stable package powerset, including bare no-default configurations, and a
metadata-derived additive accelerator row. If a later release rejects a pair, it must be
recorded symmetrically in the owning manifest, proven by a focused compile-fail
fixture, and excluded explicitly from that powerset.

Turning off `std` gives a `no_std` build of `incin-core` (with `alloc`). The
facade's backend features deliberately require `std`; a bare `incin-backends`
with no features is the backend crate's `no_std` configuration.

The executable support matrix is [`tools/feature-matrix.sh`](../../../tools/feature-matrix.sh).
Run `cargo +1.88.0 xtask feature-msrv` to compile every metadata-derived stable
combination with the workspace MSRV, or omit the tier to run every supported
compile row.
