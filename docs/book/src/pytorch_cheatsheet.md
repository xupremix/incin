...
| `training is unsupported for quantize` | The capability row declares `training = false`, which gates `ExecutionPolicy`'s training flag. | Drop that flag. It says nothing about differentiability — the STE tape node is gated by gradient recording instead (#93). |
