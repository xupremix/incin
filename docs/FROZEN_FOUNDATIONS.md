...
|---|---|
| The per-operation executor bodies in `cpu/canonical.rs` | 44 backend-executable operations still have no canonical executor. Each is additive |
...
1. **Migrate the remaining 44 backend-executable operations.** Additive, and the
