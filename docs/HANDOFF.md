...
| `crates/incin-core/src/tensor/ops/manipulation.rs` | descriptor adapters for many shape operations; split into `manipulation/{concat,indexing,interop,reshape,selectors,transfer,transpose,vision}.rs` per `docs/CONVENTIONS.md`'s file-organization convention | further split by descriptor family only if a file grows past the threshold again |
| `crates/incin-core/src/exec/catalog/attributes.rs` | the typed attribute payload for every catalog operation, plus the `AttributeContract` validation each one runs before inference sees it. One struct per operation family, so it grows by one small block whenever an operation is added rather than by any single concern getting larger | split by operation family (pooling, convolution, normalization, optimizer) if it grows again; keep each attribute struct beside its own `validate`, since the payload and the rule that admits it are one contract |
| `crates/incin-backends/src/cuda/backend/tests.rs` | the CUDA backend's own unit tests, which grew with the width-correct kernel work and the autograd recording paths; it is fixtures rather than production surface, and the suites it covers are the ones that were found asserting nothing | split by subject -- storage/contract, kernels, tape -- if it grows again; keep each test beside the invariant it pins rather than grouping by type |
| `crates/incin-core/src/exec/catalog/inference.rs` | the single place output metadata is derived from a catalog row and its operands: shape transforms, the per-output dtype rules (the position-indexed `TopK` case among them), rank and device agreement, and the operand-contract checks `verify_outputs` runs. Crossed the threshold as routing primitives were added, not by accumulating unrelated concerns | extract the shape-transform arm and the dtype-rule arm as separate files if it grows again; keep inference and verification adjacent, since a rule that derives an output and the rule that checks it must not drift apart |
...
| `crates/incin-core/src/nn/param.rs` | the two typed variable kinds and the machinery they share: `Trainable`/`Frozen` train-state markers, `ParameterInit` and the initialization-plan execution helpers, `Param` (~580 lines), and `Buffer` (~380 lines). It carries no test module, so the size is production surface rather than fixtures | `Buffer` and the initialization-plan helpers are the natural extraction targets if it grows again; keep `Param::freeze`/`unfreeze` in one file, since the train-state transition is a single visible contract and splitting it would scatter the only place trainability changes |
| `crates/incin-backends/src/codegen/ir.rs` | one expression type and everything that is defined by induction over it: the operator enums, the `IrExpr` constructors, algebraic simplification and FMA fusion, the symbolic derivative (`diff_recursive`, ~260 lines), evaluation, and CUDA expression emission (~235 lines), followed by the operator-overload and free-function DSL that builds the tree. It carries no test module, so the size is production surface. Every operator added costs one variant plus one arm in each of `apply`, `diff_recursive` and `render_cuda_expr`, which is what took it over the threshold when the transcendental and rounding operators landed | extract the symbolic-derivative arm and the CUDA emission arm as separate files if it grows again; keep the operator enums beside `apply`, since a variant whose numeric meaning lives in another file is the drift that a match arm per operator exists to prevent |
...
| `crates/incin-backends/src/dist/nccl.rs` | feature-gated distributed transport service; split into `nccl/{buffer,config,error,tests,topology,transport,wire}.rs` per `docs/CONVENTIONS.md`'s file-organization convention | further split only if a file grows past the threshold again |
| `crates/incin-backends/src/dist/tuning.rs` | coordinated two-rank collective-tuning contracts; split into `tuning/{algorithms,problem,coordination,error}.rs` per `docs/CONVENTIONS.md`'s file-organization convention | further split only if a file grows past the threshold again |
| `crates/incin-backends/src/tuning/identity.rs` | stable tuning-environment identities; split into `identity/{backend,error,primitives,device,compiler,environment,topology,cuda}.rs` per `docs/CONVENTIONS.md`'s file-organization convention | further split only if a file grows past the threshold again |
| `crates/incin-backends/src/tuning/service.rs` | feature-gated tuning service; split into `service/{context,candidate,decision,error,engine}.rs` per `docs/CONVENTIONS.md`'s file-organization convention, keeping `TuningService::decide` and `TuningPermit::finish` (two halves of one lease protocol) together in `engine.rs` rather than splitting them, since separating them would have forced roughly twenty field-visibility bumps on private types neither file needs to expose otherwise | further split only if a file grows past the threshold again |
| `crates/incin-backends/src/kernel.rs` | kernel template/rendering and specialization test vocabulary; split into `kernel/{types,scalar,packed,reduction,normalization,tests}.rs` per `docs/CONVENTIONS.md`'s file-organization convention | further split only if a file grows past the threshold again |
| `crates/incin-diagnostics/src/lib.rs` | crate root; split into sibling `humanize.rs`, `mismatch.rs`, `typenum.rs`, `tests.rs` per `docs/CONVENTIONS.md`'s file-organization convention, re-exported so the public API stays flat | further split `mismatch.rs`'s parser family only if it grows past the threshold again |
| `crates/incin-core/src/optim/mod.rs` | generic parameter optimization: update-rule traits, gradient clipping, and the concrete optimizers built on them; split into sibling `{traits,group,support,clip,optimizers}.rs` (`mod.rs` itself stays in place as the module root, alongside the pre-existing `scheduler.rs`) per `docs/CONVENTIONS.md`'s file-organization convention, keeping `SGD`/`AdamW`/`Adam` together in one `optimizers.rs` rather than one file each since all three share the same nine cross-file names and `AdamW`/`Adam` additionally share three more | further split only if a file grows past the threshold again |
| `crates/incin-backends/src/metal/backend.rs` | the Metal backend's identity, storage, capability, and executor implementations in one file; crossed 1,200 lines when the unbroadcast rank-deficit parity fix (CPU/CUDA/WGPU tails replicated for Metal) landed. The feature cannot compile on Linux, so the size is reviewed by pattern parity rather than runtime | split into `backend/{types,shape_ops,elementwise,creation,reduce,nn,contract,tests}.rs` per `docs/CONVENTIONS.md`'s file-organization convention when a macOS runner is available to verify the split |
| `crates/incin-core/src/tensor/ops/manipulation/indexing.rs` | indexing-descriptor ops (`slice`, `gather`, `scatter`, `index_select`, `masked_fill`, triangular, `chunk`/`split`/`unfold`, `where_cond`); crossed 1,200 lines when the #73 doctest sweep added runnable examples beside each op's contract docs | split by descriptor family (select vs. scatter vs. window) if it grows again; keep each doctest beside its own op |
| `crates/incin-core/src/tensor/ops/reduce.rs` | reduction descriptors and their keepdim/variant family; crossed 1,200 lines when the #73 doctest sweep added runnable examples beside each op | split by reduction family (elementwise vs. dim vs. order-statistic) if it grows again; keep each doctest beside its own op |
| `crates/incin-core/src/nn/attention.rs` | the whole attention surface in one module: `AttentionConfig`, the `AttentionBackend`/`RotaryBackend` contracts, `MultiHeadAttention` construction and its training `forward`, the fused-SDPA path, the #104 cached decode (`forward_with_cache` plus the retag/rotary helpers it shares with `forward`), and #101 `CrossAttention` (shared causal-mask/rotary helpers, `prefill_memory`, cross-attention `forward_with_cache`); grew past 2,200 lines when #101 landed | split by concern -- config/backend traits vs. `MultiHeadAttention` forward paths vs. `CrossAttention` vs. rotary/head/cmask helpers -- as the next extraction; keep each module's `forward` and `forward_with_cache` beside the shared projection and rotary code, since their absolute-position bookkeeping must stay in one place |
| `crates/incin-backends/src/cuda/ops/norm.rs` | the CUDA normalization module: the shared contiguous/dtype/device validators, the per-op geometry helpers, the layer-norm/batch-norm/rms-norm inference launchers, and their training-mode and backward counterparts with the tape recipes they feed. It crossed 1,200 lines (809 to ~1,480) when #123 added the fused batch-norm training forward (per-channel Welford with saved statistics), the fused affine backward, and the gradient-shape plan beside the inference path they share kernel-cache and precision-policy plumbing with | extract the batch-norm training/backward launchers and their geometry/plan helpers into `norm/batch_norm.rs` if it grows again; keep the shared validators and the kernel-cache plumbing in one place rather than duplicating them across the split |
| `crates/incin/src/train.rs` | the automatic `Trainer`: plan validation, the single-device `fit`/`fit_scaled` loops, loss scaling, and the post-`backward` gradient-sync seam. Crossed 1,200 lines when #97 added `GradientSynchronizer` mean-reduce and #99 added FSDP/ZeRO sharding paths (`ShardingSpec`, reduce-scatter/all-gather, stage refusals) | split into `train/{plan,loop,scale,sync,fsdp,error,tests}.rs` per `docs/CONVENTIONS.md`'s file-organization convention as the next extraction; keep the step sequence (`backward` → sync → step → gather) documented beside whichever file owns `fit`, since the order is the correctness contract |

| `crates/incin-backends/src/cuda/backend/elementwise.rs` | the CUDA elementwise module: shared launch plumbing, the unary/binary/clamp pointwise kernels' host-side launchers, tape-recording wrappers, and the pointwise gap tests' production helpers. Crossed 1,200 lines when #86 added frac/atan2/fmod/remainder tape recipes and the shared modulus-tape helper beside the existing unary/binary paths they share dtype and contiguity validators with | extract the pointwise launchers from the tape-recording wrappers into `elementwise/{launch,tape,tests}.rs` if it grows again; keep the shared validators and kernel-load plumbing in one place |
| `crates/incin-core/src/nn/state.rs` | the module state-dictionary surface: named-layer visitation, state round-trips, safetensors save/load placement hooks, and the streaming placement path added for #13 (tensor-by-tensor staging with bounded host residency and mid-load rollback). Crossed 1,200 lines when the streaming checkpoint placement landed beside the existing visit/save/load paths it shares key-order and structural-honesty checks with | split into `state/{visit,save,load,stream,tests}.rs` per `docs/CONVENTIONS.md`'s file-organization convention as the next extraction; keep the strict missing/unexpected/shape-mismatch reporting contract documented beside whichever file owns the placement loop |
| `crates/incin-backends/src/cpu/ops/shape_ops/tests.rs` | the CPU shape-ops test suite (views, slicing, transpose, fused-attention kernel pins); crossed 1,200 lines as kernel coverage accumulated beside the helpers it pins | split by subject (views vs. kernels) if it grows again; keep each test beside the invariant it pins rather than grouping by type |
| `crates/incin-core/src/tensor/device.rs` | device identity for every backend family: `DeviceKind` variants with names/ordinals/serialization, `DeviceId` constructors (incl. `External(DeviceKey)`), selectors, and the availability probes; crossed 1,200 lines when #6 added the ROCm variant and #96 added the structured device key beside the existing identity machinery | split identity types from probe/selector helpers if it grows again; keep each `DeviceKind` variant's arms adjacent, since exhaustive matches are the refusal mechanism |
| `crates/incin-core/src/serialize.rs` | checkpoint serialization: single-file and sharded safetensors envelopes, postcard/serde paths, shard-index validation (from #12), and the bounded-memory streaming placement loop (from #13) that places one tensor at a time with rollback. Crossed 1,200 lines when #13's streaming placement and its multi-shard residency tests landed beside the existing deserialize_snapshot paths they share structural honesty checks with | split into `serialize/{safetensors,postcard,stream,index,tests}.rs` per `docs/CONVENTIONS.md`'s file-organization convention; keep the shard-ownership and total_size validation beside whichever file opens the index, since those errors must name the shard |

| `crates/incin-backends/src/cpu/canonical/tests.rs` | the CPU canonical registration test suite: admission, descriptor, and coverage assertions shared by every canonical family. It is fixtures rather than production surface and grew past 1,200 lines as matmul dtype-parametricity (#90) and routing primitives added oracle rows | split by subject -- matmul, shape, reduction, capability -- if it grows again; keep each test beside the invariant it pins |
| `crates/incin-backends/src/wgpu/backend/nn.rs` | the WGPU neural-network module: normalization compositions (softmax, layer/rms/group/batch norm), loss compositions (mse/l1/bce, and cross-entropy from #91 Tier-2), dropout, linear, and the batch-norm training path. Crossed 1,200 lines when Tier-2 landed cross-entropy and batch-norm training beside the inference compositions they share statistical helpers with | split into `nn/{norm,loss,dropout,linear,tests}.rs` per `docs/CONVENTIONS.md`'s file-organization convention if it grows again; keep the shared mean/var statistical helpers in one place rather than duplicating them across norm and loss |
| `crates/incin-backends/src/metal/executor.rs` | the Metal executor: the `Execute` impls for every registered operation, dtype/layout admission guards, and the host-side tape wiring that mirrors CPU. Crossed 1,200 lines when the #92-follow-up batch (InstanceNorm, Pad/Repeat, BatchedMatMul, ArgMax/Sort/TopK, Scatter/OneHot, ToDType) landed beside the ops they share storage and refusal plumbing with. The feature cannot compile on Linux, so the size is reviewed by pattern parity rather than runtime | split into `executor/{pointwise,shape,reduce,nn,layout,loss,tests}.rs` per `docs/CONVENTIONS.md`'s file-organization convention when a macOS runner is available to verify the split |
