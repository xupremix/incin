...
| `crates/incin-core/src/nn/attention.rs` | the whole attention surface in one module: `AttentionConfig`, the `AttentionBackend`/`RotaryBackend` contracts, `MultiHeadAttention` construction and its training `forward`, the fused-SDPA path, and the #104 cached decode (`forward_with_cache` plus the retag/rotary helpers it shares with `forward`); crossed 1,200 lines when #104 landed | split by concern -- config/backend traits vs. `MultiHeadAttention` forward paths vs. rotary/head helpers -- if it grows again; keep `forward` and `forward_with_cache` beside the shared projection and rotary code, since their absolute-position bookkeeping must stay in one place |
