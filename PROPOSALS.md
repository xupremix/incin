...
| SHP-006 | core | shape | [x] | SHP-001 | `crates/incin-macros/src/rank.rs; crates/incin-core/src/shapes/` | Historical rank-ladder milestone retained for audit context; the current shape engine uses structural recursion without a semantic MAX_RANK | `cargo test -p incin-core --test rank_matrix` |
...
| EXE-005 | core | exec | [x] | EXE-001 | `crates/incin-core/src/exec/capability.rs` | Capability registry whose generated matrix matches execution tests | `cargo test -p incin-backends --no-default-features --features std,cpu --lib capability` |
...
| GRD-003 | core | grad | [x] | GRD-001 | `crates/incin-core/src/exec/tape.rs` | Backend-neutral tape nodes with CPU parity | `cargo test -p incin-backends --no-default-features --features std,cpu --lib tape` |
| GRD-004 | core | grad | [x] | GRD-003,EXE-008 | `crates/incin-backends/src/{cuda,wgpu}/` | CUDA and WGPU gradient recipes with hardware parity | `cargo test -p incin-backends --features wgpu --lib` |
...
| DST-014 | exploratory | dist | [x] | CMP-003,GRD-007,DST-008 | `crates/incin-core/src/dist/fsdp.rs` | FSDP and ZeRO prototype with persistent and transient memory parity | `cargo test -p incin --features distributed-nccl --test fsdp  # 2x networked CUDA` |
...
| D-035 | A rejected capability probe is **reported but is not a finding**. | Emitting a note per rejection, which the first draft did. `f16` matmul and `f64` reduction were unsupported on the CPU registry then (`f16` matmul has since widened, #90), so every healthy laptop opened its report with two notes about how the CPU backend simply is - not a fault, not actionable, and duplicating the probe section verbatim. |
...
| D-105 (2026-08-22) | Compiled execution for `0.1.0` is scope option 2: an unmistakable preview surface, excluded from the compatibility baseline. Feature-gated `incin::experimental::compiled`, CPU reference evaluation and plan inspection only; `CompiledArtifact` snapshots are inspection formats, not deployment targets or portable ABIs; folding, prepacking, fusion, and tuning without executable lowerings fail closed rather than claim optimization. | Option 1, a minimal stable `compile(...).run(...)` path with semantic parity and a versioned artifact contract. The source audit found capture attributes dropped, guards fabricated, folding/prepacking cloning instead of planning, fusion semantically invalid, and tuning synthetic - shipping any of that behind a stability promise would convert preview scaffolding into a support obligation. Recorded against `docs/public-api/README.md`'s baseline exclusion, the facade namespace docs, `docs/book/src/experimental.md`, `docs/PROJECT_STATUS.md`, and the release notes. |
| D-106 (2026-08-22) | The Cargo feature contract is the manifest metadata itself: every declared feature carries a `[package.metadata.incin.feature-contract]` row (tier, owner, prerequisites, hardware) or `cargo xtask docs` fails; supported combinations are the named rows in `tools/feature-matrix.sh`, the MSRV stable union is `cargo xtask feature-msrv` in CI, and powerset coverage is the cargo-hack job. No cross-feature incompatibilities are declared because the only nonsensical pairs are already structurally impossible through prerequisite feature edges (for example `hardware-tests = ["distributed-nccl"]`); if a genuine mutual exclusion ever appears it must be declared in the contract table with a trybuild fixture, which the validator already enforces symmetrically. Preview, hardware-only, and test-only tiers are outside the compatibility baseline by construction. | Maintaining a separate handwritten supported-combinations document beside the manifests. It would drift exactly like any prose table versus code (the UX-013 lesson), while the manifest-as-source design makes an unclassified feature a build failure rather than a documentation review finding. |
| D-107 (2026-08-23) | The reviewed public API for `0.1.0` is frozen as checked-in `cargo-public-api` baselines for all ten shipped packages (`docs/public-api/*.txt`, feature-pinned per crate), enforced by `tools/check-public-api-baseline.py`; any surface change requires deliberate review plus regeneration in the same commit. SemVer enforcement is `cargo-semver-checks` in CI diffing against the most recent release tag - a recorded skip until `v0.1.0` exists, then a hard gate. Preview namespaces stay out of the baselines by feature selection, not post-filtering. | Freezing only the facade (the prior state). A downstream consumer links `incin-core`, `incin-backends`, and the data/telemetry crates directly; an unfrozen core would have made the facade baseline a promise about one crate while nine others moved freely. Post-filtering generated output to hide preview items was also rejected: it lets a feature flag silently change what the filter sees, where feature selection makes exclusion structural. |
| D-108 (2026-08-23) | The doctest bar from #41 is tiered: `missing_docs` is deny-level workspace policy (summaries mandatory on every public item, all feature sets); item-level runnable examples are required for user-facing surfaces (facade, nn, tensor core methods, data, telemetry) going forward and on all new public API; internal proof machinery (`exec::catalog` internals, symbolic/proof plumbing, descriptor traits) is exempt from per-item examples because an example there restates the type signature. Module-level examples remain the norm where a crate's story needs one. | Enforcing a runnable example on every one of ~3,100 public items: most examples would restate signatures or require constructing backend fixtures inside doc tests, producing noise that rots instead of documentation that teaches. The issue itself called for exactly this per-module scoping pass. Criterion enforcement: missing_docs deny runs in every cargo invocation via workspace lints; the nightly-only rustdoc lint for missing code examples stays out of scope until stable. |
| D-109 (2026-09-24) | Broadcast-compatible shape pairs for the mask operations (#100) are expressed as directional pins on the existing SHP-007 `BroadcastShape` trait, not a new bound: `where_cond` requires `S: BroadcastShape<S2, Output = S2>` (the mask broadcasts into the data; the result keeps the data's shape type) and `masked_fill` requires `S: BroadcastShape<S2, Output = S>` (the mask broadcasts into the input; the result keeps the input's shape type). Equality is the trivial broadcast, so identical-shape call sites keep their output type; a mask that would *enlarge* the pinned operand resolves to the wrong `Output` and cannot compile when typed, and `Dyn` pairs are refused at runtime by the broadcast output rule (`mask must broadcast to the input shape` for `masked_fill`). `ShapeEq` remains the bound on the exact-match surface (`*_exact` arithmetic, comparisons, logical ops, scatter, `dot`); the broadcast arithmetic paths (`broadcast_add`, `try_*`, the `+`/`-`/`*`/`/` operators) already sat on `BroadcastShape`. Two equal runtime-axis (`usize`) types normalize per-axis to `BroadcastExtent<usize, usize>` and fail the pin where `ShapeEq` accepted them; `docs/MIGRATION.md` records spelling those axes `Dyn` or const instead. Evidence: `crates/incin/tests/broadcast_mask_ops.rs` (size-1, rank-deficit, the rank-2-mask-over-rank-4-scores acceptance case, `Dyn` rejection, backward), compile-fail `where_cond_mask_not_broadcastable`/`where_cond_mask_enlarges_data`/`masked_fill_mask_enlarges_input` and compile-pass `where_cond_broadcast_mask` under `crates/incin-core/tests/`. | A new `BroadcastCompatible<Other>` trait parallel to `BroadcastShape`: SHP-007 already carries the associated `Output` the pin equality-checks, the per-axis `BroadcastDim`/`BroadcastExtent` rule, `AppendDim`/`ReverseShape` rank promotion, the runtime `output_shape`, and the `#[diagnostic::on_unimplemented]` message, so a second trait would duplicate that rule, split the diagnostic surface, and force a migration across two bounds for one semantic. Separate `where_cond_broadcast`/`masked_fill_broadcast` methods: a second entry point per operation doubles the surface for a single bound change and leaves the default spelling wrong for the only thing these operations do with a mask - #100 was filed because the *existing* methods refused the causal `[T, T]`-over-`[B, H, T, T]` shape pair that broadcasting exists to express. Options B/C from the issue (force `Dyn` at the mask, or require an explicit `broadcast_to` at every call site) remove no friction and re-introduce the boilerplate the SHP-007 output-type preservation exists to eliminate. Widening `binary.rs`'s exact ops or `matmul.rs` in the same change: RFC recommended scope defers them until the new bound's diagnostics are judged on a real attention implementation, `*_exact` is deliberately exact, and MatMulShape encodes alignment rules broadcasting does not model. |
| D-110 (2026-09-25) | The `TensorElement` seal is lifted and third-party device identity is a structured key (#96). Seal: UNSEAL with the POD bound - `sealed::TensorElementSealed` is deleted as a bound component (and with it the per-type seal impls), keeping `bytemuck::NoUninit + Zeroable + Copy + Debug + Send + Sync + 'static` as the proof obligation any POD newtype, including foreign-crate types, satisfies through the blanket impl; the bound was policy, not safety (`f16`/`bf16` already prove non-primitives qualify). Preconditions, all landed with the widening: (a) the NCCL `builtin_id().unwrap_or(DTypeId::F32)` error-field coercions (`dist/nccl/transport.rs`, data-parallel / tensor-parallel / pipeline entry points) now refuse unknown descriptors with `InvalidBuffer` naming the `namespace/name@version` key instead of mislabeling the refusal `F32`; (b) every `TensorElement`/`PlainDType` consumer audited - `to_vec1`/`to_scalar` keep their closed `TypeId` allow-lists (typed refusal for unknown `E`), `scalar_type_matches_dtype` keeps its `_ => false` arm, `from_slice`'s byte path is generic over `Elem`, and `to_vec_elem` takes a size contract against the descriptor encoding for non-allow-list `Elem` (built-ins behaviorally unchanged, `bool` keeps per-byte validation); (c) `BuiltinDType` gates stay in the four closed subsystems (distributed static planners/digests, kernel-table fast paths, catalog exact rules, `safetensors` table). Device identity: two tiers - maintained backends keep first-class `DeviceKind` variants, third-party backends get `DeviceKind::External(DeviceKey)` (`namespace + name + version`, paralleling `DTypeKey`), replacing the opaque `DeviceKind::Custom(u64)` (removed pre-1.0 with a `docs/MIGRATION.md` note); `name()` reports the key's device name, plan digests hash namespace + version beside it, and device sets / mesh binding compare the whole key. Consequence for #94: FP8 follows the `bf16` precedent (real element type). Evidence: `crates/incin-core/tests/custom_element_proof.rs` (in-test 3-byte posit-like `PositElem`/`Posit24` + byte-store backend: `from_slice`/`to_vec_elem` round-trip, CPU `UnsupportedDType` refusal carrying the descriptor, manifest refusal naming the key), `tensor_element_requires_pod` compile-fail (POD obligation still gates) and `custom_pod_element` compile-pass fixtures, `DeviceKey`/digest/`DeviceSet` unit and `mesh_bind` external cases. | Keeping the seal (re-justifying a policy refusal per format, and forcing per-dtype holes worse than a principled bound); keeping `Custom(u64)` (opaque integers collide by convention and cannot version); widening any `BuiltinDType` gate in the same change (descriptor-keyed digests, mixed-version peer refusal, and catalog exact-rule coverage are the future phase, not this one); deprecating instead of removing `Custom(u64)` (two vocabularies for one device identity, against D-008). |
| D-111 (2026-09-25) | The MoE public signature keeps one static-typed path for both the B-interim (Dyn span) and C-target (offset-array grouped execution) strategies (#102): `DroplessMoE<const E: usize, const TOPK: usize, B, K, Train, Expert = ExpertMlp>` implements `Module<Tensor<Dyn, B, K, G>>` with `Output = (combined Dense<Dyn, B, K, Grad>, aux scalar)` - static `[T, D]` in/out, the data-dependent span kept inside as `[E+1]` offsets over a static `[T*K, D]` buffer (option C), never as a type; `E`/`TOPK` are const generics, aux loss travels in `Output` (never a side channel), gradients are gate-weight-only (indices/permutation/offsets/counts are `NoGrad` by type, no straight-through). The CPU prototype (`incin::experimental::{DroplessMoE, ExpertMlp, GroupedExpert, DroplessMoEBackend}`, `crates/incin-core/src/moe_dropless.rs`) runs option-C geometry (router, offsets, one `grouped_matmul` per projection, scatter-add combine) with documented deviations: `Dyn` (not static `S`) input because #100's index-pair bounds and a static `grouped_matmul` overload are still open; combine seed requires grad (scatter_add records under the base marker); one host readback for the scatter index; CPU-effective device bound (`Arg = ()`); expert gradients need grad-marked input (grouped_matmul records from lhs only - a genuine B-vs-C gap vs dense masked MoE, recorded not hidden). Evidence: `crates/incin/tests/moe_dropless.rs` (shapes/aux, determinism, gate grads via output and aux, expert grads, NoGrad indices by type, save/load roundtrip, E-mismatch refusal) plus the worked example `crates/incin/examples/moe_dropless_grouped.rs`. | Naming the dynamic span in the type system (existentials cannot name `Module::Output` or compose via `Sequential`; const-threading needs unstabilized `generic_const_exprs` - see `102-nameable-spans.md`); capacity-factor padding as the default (silent token dropping as framework behavior); a separate signature per strategy (breaks every model at the interim/target swap); aux loss in a side channel or caller-added method (silently forgettable, and changes `Module::Output` later anyway). |
