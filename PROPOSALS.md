...
| EXE-002 | core | exec | [ ] | EXE-001 | `crates/incin-core/src/exec/proof.rs` | Sealed Validated<O> and proof provenance, with privacy compile-fail tests and the paranoid-validation feature | `cargo test -p incin-core --test compile_tests` |
| EXE-003 | core | exec | [ ] | EXE-002 | `crates/incin-core/src/exec/rule.rs` | ShapeRule lowering for broadcast, reduction, reshape, matmul, conv, and pool, each restating its frontend trait Output | `cargo test -p incin-core --test lowering_parity` |
...
| MTL-002 | preview | metal | [ ] | MTL-001,EXE-003 | `crates/incin-backends/src/metal/shaders/` | Generated MSL pointwise and reduction descriptors with parity tests | `cargo test -p incin-backends --features metal  # Apple Silicon` |
| MTL-003 | preview | metal | [ ] | MTL-001,PRF-004 | `crates/incin-backends/src/metal/mps.rs` | MPS and MPSGraph structured candidates with explicit native fallback | `cargo test -p incin-backends --features metal-mps  # Apple Silicon` |
...
| `ShapeRule` | `incin-core::exec::rule` | EXE-003 | companion to `BroadcastShape`, `MatMulShape`, … (see §1.2.1) |
