//! Tests for the fused AdamW optimizer step kernel.
...
fn test_adamw_analytical_formula() {
    ...
    // Verify first element math
    let expected_m0 = 0.1 * (1.0 - 0.9);
    assert!((m_out[0] - expected_m0).abs() < 1e-6);
    assert!(p_out[0] < p_in[0]); // gradient was positive, so parameter moved in negative direction
}
