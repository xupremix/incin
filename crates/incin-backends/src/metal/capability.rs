...
use incin_core::prelude::{DType, DTypeDescriptor, DTypeId, Device, DeviceId, Error, OperationKind, Result};
...
pub(crate) fn validate_metal_storage_dtype(dtype: DTypeDescriptor, op: &'static str) -> Result<()> {
    if matches!(dtype.builtin_id(), Some(DTypeId::F32 | DTypeId::F64 | DTypeId::F16 | DTypeId::BF16 | DTypeId::I64 | DTypeId::Q8_0)) {
        Ok(())
    } else {
        Err(Error::UnsupportedDType { dtype, backend: "Metal", op })
    }
}
...
pub(crate) fn native_precision(
    ...
    let accumulator = match request.operation {
        OperationKind::Reduction | OperationKind::Normalization
            if matches!(request.storage.builtin_id(), Some(DTypeId::F16 | DTypeId::BF16)) => DTypeId::F32.descriptor(),
        _ => compute,
    };
    Ok(incin_core::exec::ResolvedPrecision::new(request.storage, compute, accumulator, request.output, incin_core::exec::LossScaling::None))
}

impl<D: Device> incin_core::exec::PrecisionCapabilities for super::backend::MetalBackendImpl<D> {
    fn native_precision(&self, request: &incin_core::exec::PrecisionRequest) -> Result<incin_core::exec::ResolvedPrecision> {
