...
use super::helpers::{natural_transpose_out_size, with_batch_axis, without_batch_axis};
...
pub(crate) fn conv_transpose2d_impl<D: incin_core::tensor::device::Device, K: DType>(
    ...
    // batch of one here and has it taken back off below, so the fold arithmetic
    // and the tape recipe stay written against one shape.
    let (activation, unbatched) = with_batch_axis("conv_transpose2d", input, 4)?;
    let (b, cin, h, w) = (
        activation.shape[0],
        activation.shape[1],
        activation.shape[2],
    ...
    let input_flat = activation.reshape(&[b, cin, input_spatial])?;
    ...
    tape::push_with(|| TapeEntry {
        ...
        backward: Box::new(move |grad_out: &CpuStorage| {
            ...
            // The batch axis goes back on first when the activation had none,
            // since the gradient is shaped like the output and the output lost
            // it, and every step below addresses axes two and three.
            let grad_out = with_batch_axis("conv_transpose2d", grad_out, 4)?.0;
            ...
            // Shaped like the activation that was asked about, not like the
            // batched one the kernel worked on.
            let grad_input = grad_input_flat.transpose(1, 2)?.reshape(&input_shape)?;
