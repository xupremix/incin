...
fn main() -> Result<()> {
    ...
    println!(
        "Processed 10,000 items in parallel in {:?}",
        start.elapsed()
    );
    println!("Sum: {}", sum);

    println!(
        "\n(Note: HuggingFace hub loading works asynchronously via `HuggingFaceHub::load_safetensors`, see `crates/incin-data/src/hf.rs` for the async API!)"
    );
