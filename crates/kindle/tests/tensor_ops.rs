...
fn test_binary_add() -> Result<()> {
    ...
    let b = Tensor::<s![4], CpuBackend>::from_slice(&[2.0, -2.0, 0.0, 3e38], ())?;
...
fn test_binary_sub() -> Result<()> {
    ...
    let b = Tensor::<s![3], CpuBackend>::from_slice(&[2.0, 4.0, 3.0], ())?;
...
fn test_binary_mul() -> Result<()> {
    ...
    let b = Tensor::<s![3], CpuBackend>::from_slice(&[5.0, 1.0, 3.0], ())?;
...
fn test_binary_div() -> Result<()> {
    ...
    let b = Tensor::<s![3], CpuBackend>::from_slice(&[2.0, 0.0, 1e20], ())?;
...
fn test_indexing_stack() -> Result<()> {
    ...
    let c_slice = Tensor::<s![2], CpuBackend>::try_concat_slice(&[&t1, &t2, &t1], 0)?;
...
fn test_loss_mse() -> Result<()> {
    ...
    let target2 = Tensor::<s![2], CpuBackend>::from_slice(&[-1.0, -2.0], ())?; // different
    let target3 = Tensor::<s![2], CpuBackend>::from_slice(&[1.1, 1.9], ())?; // small deltas
