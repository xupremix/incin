...
fn test_linear_mixed_shapes() {
    ...
    // 2. Fully dynamic
    let _ = Linear::<s![dyn, dyn], B>::new_dyn((3, 4)).unwrap();

    // 3. Partially static (In static, Out dynamic)
    let _ = Linear::<s![3, dyn], B>::new_dyn(4).unwrap();

    // 4. Partially static (In dynamic, Out static)
    let _ = Linear::<s![dyn, 4], B>::new_dyn(3).unwrap();
}
...
fn test_conv2d_mixed_shapes() {
    ...
    // 2. Fully dynamic
    let _ = Conv2d::<s![dyn, dyn, 3, 1, 1, 1], B>::new_dyn((3, 4)).unwrap();

    // 3. Partially static (Out dynamic, In static)
    let _ = Conv2d::<s![dyn, 4, 3, 1, 1, 1], B>::new_dyn(3).unwrap();

    // 4. Partially static (Out static, In dynamic)
    let _ = Conv2d::<s![3, dyn, 3, 1, 1, 1], B>::new_dyn(4).unwrap();
}
