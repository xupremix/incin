//! Device-to-backend selection for the unified `KindleBackend` facade.

#[cfg(feature = "cpu")]
use kindle_core::prelude::Cpu;
use kindle_core::prelude::{Backend, DType, Device, Dyn};
...
///
/// `KindleBackend<T, D>` is a type alias through this trait, so it retains
...
#[cfg(feature = "wgpu")]
impl<T: DType, const N: usize> sealed::Sealed<T> for kindle_core::prelude::Wgpu<N> {}

#[cfg(feature = "wgpu")]
impl<T: DType, const N: usize> BackendFor<T> for kindle_core::prelude::Wgpu<N> {
    type Backend = crate::wgpu::WgpuBackendImpl<T, kindle_core::prelude::Wgpu<N>>;
}

#[cfg(feature = "cuda")]
impl<T: DType, const N: usize> sealed::Sealed<T> for kindle_core::prelude::Cuda<N> {}

#[cfg(feature = "cuda")]
impl<T: DType, const N: usize> BackendFor<T> for kindle_core::prelude::Cuda<N> {
    type Backend = crate::cuda::CudaBackendImpl<T, kindle_core::prelude::Cuda<N>>;
}
...
macro_rules! impl_transfer {
    ($source:ty) => {
        impl<T: DType, D: Device, NewD> kindle_core::prelude::TransferTo<NewD> for $source
        where
            NewD: BackendFor<T>,
        {
            type Output = crate::KindleBackend<T, NewD>;

            fn transfer_storage<K: DType>(
                storage: &Self::Storage<K>,
                dtype: &K::Field,
                device: &NewD::Field,
            ) -> kindle_core::prelude::Result<<Self::Output as Backend>::Storage<K>>
            where
                Self::Output: kindle_core::prelude::SupportsDType<K>,
            {
                use kindle_core::prelude::{Error, SupportsDType};
                let expected_dtype = K::to_kindle(dtype);
            ...
            fn transfer_var(
                ...
                device: &NewD::Field,
            ) -> kindle_core::prelude::Result<<Self::Output as Backend>::RawVar>
            where
                Self::Output: kindle_core::prelude::SupportsDType<T>,
            {
                use kindle_core::prelude::SupportsDType;
                let source = Self::var_as_tensor::<T>(variable)?;
                let expected_dtype = T::to_kindle(dtype);
                if let Some(got) = Self::storage_dtype::<T>(&source)
                    && got != expected_dtype
                {
                    return Err(kindle_core::prelude::Error::DTypeStorageMismatch {
                        expected: expected_dtype,
                        got,
                    });
                }
...
mod tests {
    ...
    fn selects_cpu_backend() {
        assert_backend::<crate::KindleBackend<f32, Cpu>>();
    }

    #[cfg(feature = "wgpu")]
    #[test]
    fn selects_wgpu_backend() {
        assert_backend::<crate::KindleBackend<f32, kindle_core::prelude::Wgpu>>();
    }

    #[cfg(feature = "cuda")]
    #[test]
    fn selects_cuda_backend() {
        assert_backend::<crate::KindleBackend<f32, kindle_core::prelude::Cuda>>();
    }

    #[cfg(feature = "cpu")]
    #[test]
    fn runtime_dispatch_selects_cpu_and_preserves_metadata() {
        type B = crate::KindleBackend<Dyn, Dyn>;
    ...
    fn static_cpu_transfer_rebuilds_dynamic_dispatch_storage() {
        type Source = crate::KindleBackend<f32, Cpu>;
        type Target = crate::KindleBackend<f32, Dyn>;
