...
use kindle_core::prelude::{CreationOps, DType, DTypeId, DeviceId, Result};
...
fn fill_buffer(total: usize, value: f64, dtype: DTypeId, device: &DeviceId) -> Result<CpuBuffer> {
    ...
    match device.kind() {
        kindle_core::prelude::DeviceKind::Cpu => Ok(host_buf),
...
impl<T: DType, D: Device> CreationOps<Self> for CpuBackendImpl<T, D> {
    ...
    fn rand<K: DType>(
        ...
        let final_buffer = match device.kind() {
            kindle_core::prelude::DeviceKind::Cpu => buffer,
