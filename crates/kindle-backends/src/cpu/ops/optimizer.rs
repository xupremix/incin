    ...
    OptimizerOps<CpuBackend<T, D>> for CpuBackend<T, D>
{
    ...
    fn adamw_step<K: DType>(
        ...
        #[cfg(all(feature = "cuda", feature = "fused"))]
        {
            ...
            if let (
                CpuBuffer::Cuda(v_buf),
                CpuBuffer::Cuda(g_buf),
                CpuBuffer::Cuda(_m_buf),
                CpuBuffer::Cuda(_v2_buf),
            ) = (
                &*var_storage.buffer,
                &*_grad.buffer,
                &*_m.buffer,
                ...
                let module = crate::cpu::gpu::cuda_cache::get_module(v_buf.device_id, "kindle_cuda")
                    .expect("Module kindle_cuda not found");
                ...
                let mut new_dev_slice = stream
                    .alloc_zeros::<u8>(num_elements * core::mem::size_of::<f32>())
                    .unwrap();
                let mut new_v_f32 = unsafe {
                    new_dev_slice.transmute_mut::<f32>(num_elements).ok_or(
                        kindle_core::prelude::Error::UnsupportedBackendOperation {
                            op: "adamw",
                            backend: "Cuda requires f32",
                        },
                    )?
                };
                ...
                let v_f32 = unsafe {
                    v_buf.data.transmute::<f32>(num_elements).ok_or(
                        kindle_core::prelude::Error::UnsupportedBackendOperation {
                            op: "adamw",
                            backend: "Cuda requires f32",
                        },
                ...
                let g_f32 = unsafe {
                    g_buf.data.transmute::<f32>(num_elements).ok_or(
                        kindle_core::prelude::Error::UnsupportedBackendOperation {
                            op: "adamw",
                            backend: "Cuda requires f32",
                        },
                ...
                let m_buffer =
                    alloc::sync::Arc::get_mut(&mut m.buffer).expect("m is not uniquely owned");
                let v2_buffer =
                    alloc::sync::Arc::get_mut(&mut v.buffer).expect("v is not uniquely owned");

                if let (CpuBuffer::Cuda(m_buf_mut), CpuBuffer::Cuda(v2_buf_mut)) =
                    (m_buffer, v2_buffer)
                {
                    let m_slice_u8: &mut cudarc::driver::CudaSlice<u8> =
                        alloc::sync::Arc::get_mut(&mut m_buf_mut.data)
                            .expect("Failed to get mut to m buffer data");
                    let mut m_f32 = unsafe {
                        m_slice_u8.transmute_mut::<f32>(num_elements).ok_or(
                            kindle_core::prelude::Error::UnsupportedBackendOperation {
                                op: "adamw",
                                backend: "Cuda requires f32",
                            },
                    ...
                    let v2_slice_u8: &mut cudarc::driver::CudaSlice<u8> =
                        alloc::sync::Arc::get_mut(&mut v2_buf_mut.data)
                            .expect("Failed to get mut to v buffer data");
                    let mut v2_f32 = unsafe {
                        v2_slice_u8.transmute_mut::<f32>(num_elements).ok_or(
                            kindle_core::prelude::Error::UnsupportedBackendOperation {
                                op: "adamw",
                                backend: "Cuda requires f32",
                            },
                    ...
                    let new_var_buf = crate::cpu::storage::CpuCudaBuffer {
                        len: num_elements,
                    ...
                    let updated_storage = crate::cpu::storage::CpuStorage {
                        buffer: alloc::sync::Arc::new(CpuBuffer::Cuda(new_var_buf)),
                    ...
                    crate::cpu::var::assign_var(var, &updated_storage)?;
                    return Ok(());
                }
            }
        }
