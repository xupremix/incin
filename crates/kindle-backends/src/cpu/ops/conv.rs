...
fn cuda_im2col_1d(
    ...
    if let CpuBuffer::Cuda(buf) = &*input.buffer {
        ...
        if crate::cpu::gpu::cuda_cache::get_module(device_id, "conv").is_none() {
        ...
        let mut out_b = crate::cpu::storage::CpuCudaBuffer {
            len: out_len,
...
fn cuda_col2im_1d(
    ...
    if let CpuBuffer::Cuda(buf) = &*cols_grad.buffer {
        let device_id = buf.device_id;
        let stream = buf.device.default_stream();
        let dispatcher = crate::cpu::gpu::CpuCudaDispatcher::new(device_id);
...
fn im2col_1d(
    ...
    #[cfg(feature = "cuda")]
    if let CpuBuffer::Cuda(_) = &*input.buffer {
...
fn col2im_1d(
    ...
    #[cfg(feature = "cuda")]
    if let CpuBuffer::Cuda(_) = &*cols_grad.buffer {
