...
use kindle_core::prelude::{DTypeId, Error, Result};
...
fn render_cuda(
    ...
    let entry_point = format!(
        "kindle_{family}_{}_u{unroll_width}_{op_name}",
        scalar.suffix
    );
    let source = template
...
pub(crate) fn render_cuda_normalization(op_name: &str, dtype: DTypeId) -> Result<RenderedKernel> {
    ...
    let entry_point = format!("kindle_normalization_{}_{}", scalar.suffix, op_name);
    let key = KernelKey::cuda(
        OperationFamily::Normalization,
    ...
    let source = match op_name {
        "layer_norm" => format!(
            r#"
{preamble}
struct KindleWelford {{
    {compute_type} mean;
    {compute_type} m2;
    int count;
}};

__device__ __forceinline__ KindleWelford kindle_welford_combine(
    KindleWelford left, KindleWelford right)
{{
...
{{
    ...
    int lane = tid & 31;
    int warp = tid >> 5;
    KindleWelford local = {{({compute_type})0.0, ({compute_type})0.0, 0}};
    ...
    for (int delta = 16; delta > 0; delta >>= 1) {{
        ...
        if (lane + delta < 32) local = kindle_welford_combine(local, other);
    }}
    extern __shared__ unsigned char shared_raw[];
    int warp_count = (blockDim.x + 31) >> 5;
...
mod tests {
    ...
    fn normalization_templates_use_welford_and_dtype_specific_compute() {
        ...
        assert!(half.source.contains("struct KindleWelford"));
