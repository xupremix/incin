...
/// Cbackend.
type CBackend = kindle_backends::cpu::CpuBackendImpl;

#[test]
/// Test slice.
fn test_slice() {
    let t = CBackend::zeros::<f32>(&[4, 4], DTypeId::F32, &DeviceId::cpu()).unwrap();
...
/// Test reshape.
fn test_reshape() {
    let t = CBackend::zeros::<f32>(&[2, 8], DTypeId::F32, &DeviceId::cpu()).unwrap();
...
/// Test max pool2d.
fn test_max_pool2d() {
    let t = CBackend::zeros::<f32>(&[1, 3, 16, 16], DTypeId::F32, &DeviceId::cpu()).unwrap();
...
fn test_conv2d() {
    let t = CBackend::zeros::<f32>(&[1, 3, 16, 16], DTypeId::F32, &DeviceId::cpu()).unwrap();
    let w = CBackend::zeros::<f32>(&[8, 3, 3, 3], DTypeId::F32, &DeviceId::cpu()).unwrap();
...
fn test_batch_norm() {
    let t = CBackend::zeros::<f32>(&[1, 3, 16, 16], DTypeId::F32, &DeviceId::cpu()).unwrap();
    let w = CBackend::ones::<f32>(&[3], DTypeId::F32, &DeviceId::cpu()).unwrap();
    let b = CBackend::zeros::<f32>(&[3], DTypeId::F32, &DeviceId::cpu()).unwrap();
    let rm = CBackend::zeros::<f32>(&[3], DTypeId::F32, &DeviceId::cpu()).unwrap();
    let rv = CBackend::ones::<f32>(&[3], DTypeId::F32, &DeviceId::cpu()).unwrap();
