...
#[test]
fn test_max_pool2d() {
    let t = CBackend::zeros::<f32>(&[1, 3, 16, 16], KindleDType::F32, &KindleDevice::cpu()).unwrap();
...
fn test_batch_norm() {
    ...
    let bn = CBackend::batch_norm::<f32>(&t, Some(&w), Some(&b), Some(&rm), Some(&rv), 1e-5, 0.1).unwrap();
