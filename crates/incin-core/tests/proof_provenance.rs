...
fn a_shape_written_entirely_in_typenum_is_fully_proved() {
    assert_eq!(ProofLevel::of::<(U3, U4)>(), ProofLevel::Static);
    assert!(ProofLevel::of::<(U3, U4)>().is_static());
    assert!(ProofLevel::of::<(U3, U4)>().has_static_rank());
}
...
fn a_single_runtime_axis_is_enough_to_weaken_the_shape() {
    ...
    assert_eq!(ProofLevel::of::<(U3, usize)>(), ProofLevel::Mixed);
    assert_eq!(ProofLevel::of::<(usize, U3, U4)>(), ProofLevel::Mixed);
}
...
fn naming_an_axis_types_it_without_sizing_it() {
    ...
    assert_eq!(ProofLevel::of::<(Batch, U4)>(), ProofLevel::Mixed);
    assert_eq!(ProofLevel::of::<(Batch, Seq)>(), ProofLevel::Mixed);
}
...
fn a_rank_one_static_shape_is_still_static() {
    ...
    assert_eq!(ProofLevel::of::<(U1,)>(), ProofLevel::Static);
}
...
fn an_operation_is_only_as_proved_as_its_weakest_operand() {
    let lhs = ProofLevel::of::<(U3, U4)>();
    let rhs = ProofLevel::of::<(U3, usize)>();
...
fn folding_three_operands_matches_the_conv_case_the_rfc_could_not_express() {
    ...
    let input = ProofLevel::of::<(usize, U3, U4, U4)>();
    let weight = ProofLevel::of::<(U4, U3, U1, U1)>();
    let bias = ProofLevel::of::<(U4,)>();
...
fn of_is_usable_in_a_const_so_a_backend_can_branch_at_compile_time() {
    ...
    const STATIC_PAIR: ProofLevel = ProofLevel::of::<(U3, U4)>();
    const MIXED_PAIR: ProofLevel = ProofLevel::of::<(U3, usize)>();
