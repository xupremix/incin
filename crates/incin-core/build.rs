...
fn main() -> Result<()> {
    ...
    if let Err(e) = config.compile_protos(&["proto/onnx.proto"], &["proto/"]) {
        ...
        eprintln!(
            "Incin Core Build Error: Failed to compile ONNX Protocol Buffers (proto/onnx.proto)."
        );
        eprintln!("Reason: {}", e);
        eprintln!(
            "================================================================================"
        );
        eprintln!("`protoc` (Protocol Buffers Compiler) is required to build incin-core.");
