//! `kindle-lsp` — spawns rust-analyzer as a child process and pumps LSP
//! frames between it and the editor on two threads, rewriting only the
//! frames `kindle_lsp::rewrite` recognizes. See `kindle_lsp` (the library
//! crate) for the architectural rationale.

use kindle_lsp::config::Config;
use kindle_lsp::frame::{read_frame, write_frame};
use kindle_lsp::rewrite::{PendingRequests, rewrite_incoming_from_server};
...
fn main() -> io::Result<()> {
    ...
    let mut child = Command::new(&config.ra_path)
        ...
        .spawn()
        .unwrap_or_else(|e| {
            eprintln!("kindle-lsp: failed to spawn '{}': {e}", config.ra_path);
            std::process::exit(1);
        });
