//! `kindle-lsp` — a transparent LSP proxy that sits between an editor and
//! rust-analyzer: it spawns rust-analyzer as a child process and rewrites
//! only two message kinds through `kindle-diagnostics` before forwarding
//! them on —
//!
...
//!   treatment via [`kindle_diagnostics::humanize_inlay_label`].
//!
