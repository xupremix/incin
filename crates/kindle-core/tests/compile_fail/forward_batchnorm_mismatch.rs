...
fn main() {
    ...
    let layer: BatchNorm2d<U32, DummyBackend<f32, Cpu>> = BatchNorm2d::new((), &device).unwrap();
    
    // Tensor has 16 channels, this should fail!
    let tensor: Tensor<(usize, U16, usize, usize), DummyBackend<f32, Cpu>> = Tensor::zeros((1, 1, 1)).unwrap();
