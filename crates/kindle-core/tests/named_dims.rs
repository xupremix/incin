...
kindle_core::symbolic_dim!(Batch, Feature);
...
kindle_core::symbolic_dim!(Channel);
...
fn nn_conv2d_layer_preserves_a_named_batch_dim() {
    ...
    let conv = Conv2d::<ConvShape, kindle_backends::cpu::CpuBackendImpl>::build(()).unwrap();
    let x: Tensor<s![Batch, 3, 8, 8], kindle_backends::cpu::CpuBackendImpl> =
        Tensor::zeros((2usize,)).unwrap();
    let out: Tensor<s![Batch, 5, 6, 6], kindle_backends::cpu::CpuBackendImpl> =
