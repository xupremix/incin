...
fn test_linear_permutations() {
    ...
    let _l3b = Linear::<s![10, 20], B, Dyn>::new_with(((), ()), false).unwrap();

    // 5. Partially static (In static, Out dynamic), Bias = True
    let _l4 = Linear::<s![10, dyn], B>::new_with(((), 20)).unwrap();

    // 6. Partially static (In dynamic, Out static), Bias = True
    let _l5 = Linear::<s![dyn, 20], B>::new_with((10, ())).unwrap();
...
fn test_conv1d_permutations() {
    ...
    let _c6 = Conv1d::<s![dyn, 3, 3, 1, 1, 1], B>::new_with((16, ())).unwrap();
}
...
fn test_conv2d_permutations() {
    ...
    // 7. Partially dynamic (Out dynamic, In static)
    let _c6 = Conv2d::<s![dyn, 3, 3, 1, 1, 1], B>::new_with((16, ())).unwrap();
}
...
fn test_norm_permutations() {
    ...
    // BatchNorm2d — static
    let _bn1 = BatchNorm2d::<s![16], B>::new(1e-5, 0.1).unwrap();
    // BatchNorm2d — dynamic
...
fn test_rnn_permutations() {
    ...
    let _wh = Linear::<s![20, 20], B>::new().unwrap();
