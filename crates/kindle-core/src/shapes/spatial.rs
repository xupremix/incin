...
impl<B: Dim, C: Dim, HIn: Dim, WIn: Dim, K: Unsigned, S: Unsigned> Pool2dShape<K, S> for (B, C, HIn, WIn)
where
...
impl<Batch: Dim, CIn: Dim, COut: Dim, LIn: Dim, K: Unsigned, S: Unsigned, P: Unsigned, D: Unsigned> Conv1dShape<K, S, P, D> for (Batch, CIn, LIn, COut)
where
    ...
    <LIn as SpatialOut<K, S, P, D>>::Output: Dim,
{
    type Output = (
        Batch,
        COut,
        <LIn as SpatialOut<K, S, P, D>>::Output,
    );
}
...
impl<Batch: Dim, CIn: Dim, COut: Dim, HIn: Dim, WIn: Dim, K: Unsigned, S: Unsigned, P: Unsigned, D: Unsigned> Conv2dShape<K, S, P, D> for (Batch, CIn, HIn, WIn, COut)
where
