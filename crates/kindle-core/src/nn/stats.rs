...
#[cfg(test)]
mod tests {
    // The `s!` macro's expansion hardcodes `kindle::...` paths (it's
    // written for the `kindle` facade crate's consumers, not for
    // `kindle-core`'s own internal tests) — this alias is what every real
    // (non-doc-comment) `s![...]` usage in this codebase relies on,
    // otherwise done via `extern crate kindle_core as kindle;` in the
