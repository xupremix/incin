...
/// shared `Rc<NativeBuffer>` in place (T-02-01 mitigation) — always
/// constructs a new `Vec` copy first.
fn perturbed(storage: &NativeStorage, flat_idx: usize, delta: f64) -> NativeStorage {
    ...
    for _ in 0..flat_idx {
        crate::ops::elementwise::increment_index(&mut idx, shape);
    }
...
fn numerical_grad(
    f: &impl Fn(&[NativeStorage]) -> NativeStorage,
    inputs: &[NativeStorage],
    input_idx: usize,
    flat_idx: usize,
    eps: f64,
) -> f64 {
    let mut plus: Vec<NativeStorage> = inputs.to_vec();
    let mut minus: Vec<NativeStorage> = inputs.to_vec();
...
pub fn gradcheck(
    op: impl Fn(&[NativeStorage]) -> NativeStorage,
    inputs: &[NativeStorage],
    eps: f64,
) -> f64 {
    let out = op(inputs);
