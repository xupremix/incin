...
pub fn launch_nll_loss(
    log_sm: &NativeStorage,
    targets: &NativeStorage,
    classes: usize,
) -> Result<NativeStorage> {
    if let (NativeBuffer::Cuda(b_log_sm), NativeBuffer::Cuda(b_targets)) = (&*log_sm.buffer, &*targets.buffer) {
        ...
        if crate::gpu::cuda_cache::get_module(device_id, kernel_name).is_none() {
            ...
            dispatcher.compile_and_load_kernel(kernel_name, crate::ops::cuda_kernels::LOSS_KERNEL, kernel_name)?;
        }
