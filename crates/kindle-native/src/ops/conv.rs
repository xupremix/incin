//! `conv1d`/`conv2d` for `NativeBackend<T, D>` via im2col (window-unfold into
...
//! `NativeStorage` view op, per Pitfall 2): every gathered element whose
...
//! functions operating on already-materialized `NativeStorage` values, so
...
fn im2col_1d(
    ...
    NativeStorage::from_contiguous(NativeBuffer::F32(out), vec![b, l_out, cin * kernel_size])
}
...
fn col2im_1d(
    ...
    let out_strides = crate::stride::contiguous_strides(input_shape);
    ...
    NativeStorage::from_contiguous(NativeBuffer::F32(out), input_shape.to_vec())
}
...
/// `ModuleOps::conv1d`'s NativeBackend implementation: im2col + per-group
...
pub(crate) fn conv1d_impl<T: DType, D: kindle_core::prelude::Device, K: DType>(
    input: &NativeStorage,
    ...
    let mut group_outputs: Vec<NativeStorage> = Vec::with_capacity(groups);
    for g in 0..groups {
    ...
    let refs: Vec<&NativeStorage> = group_outputs.iter().collect();
    ...
    tape::push(TapeEntry {
        ...
        backward: Box::new(move |grad_out: &NativeStorage| {
            ...
            let mut grad_input_groups: Vec<NativeStorage> = Vec::with_capacity(groups);
            let mut grad_weight_groups: Vec<NativeStorage> = Vec::with_capacity(groups);
            for g in 0..groups {
...
/// `ModuleOps::conv2d`'s NativeBackend implementation, mirroring
...
fn sum_batch_dim(t: &NativeStorage) -> NativeStorage {
    ...
    NativeStorage::from_contiguous(NativeBuffer::F32(out), vec![m, n])
}
...
fn concat_along_dim1(parts: &[NativeStorage]) -> NativeStorage {
...
fn concat_along_dim0(parts: &[NativeStorage]) -> NativeStorage {
...
fn concat_along_dim(parts: &[NativeStorage], dim: usize) -> NativeStorage {
    ...
    NativeStorage::from_contiguous(NativeBuffer::F32(out), out_shape)
}
