...
impl<T: kindle_core::prelude::DType, D: kindle_core::prelude::Device + Clone + 'static>
    OptimizerOps<NativeBackend<T, D>> for NativeBackend<T, D>
{
    ...
    fn adamw_step<K: DType>(
        ...
        {
            let var_storage = crate::var::var_as_tensor(_var)?;
            if let (
                NativeBuffer::Cuda(v_buf),
                NativeBuffer::Cuda(g_buf),
                NativeBuffer::Cuda(_m_buf),
                NativeBuffer::Cuda(_v2_buf),
            ) = (
                &*var_storage.buffer,
                &*_grad.buffer,
                &*_m.buffer,
                ...
                let module = crate::gpu::cuda_cache::get_module(v_buf.device_id, "kindle_cuda")
                ...
                if let (NativeBuffer::Cuda(m_buf_mut), NativeBuffer::Cuda(v2_buf_mut)) =
                    (m_buffer, v2_buffer)
                {
