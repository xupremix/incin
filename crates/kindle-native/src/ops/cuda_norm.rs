...
use crate::ops::cuda_kernels::NORM_KERNEL;
use cudarc::driver::PushKernelArg;

#[cfg(feature = "cuda")]
fn ensure_norm_loaded(device_id: usize) -> Result<()> {
    if crate::gpu::cuda_cache::get_module(device_id, "norm").is_none() {
...
pub fn launch_layer_norm(
    ...
    unsafe {
        ...
        if let Some(b) = bias {
            if let NativeBuffer::Cuda(b_buf) = &*b.buffer {
...
pub fn launch_batch_norm(
    ...
    if crate::gpu::cuda_cache::get_module(device_id, "batch_norm").is_none() {
        let dispatcher = crate::gpu::NativeCudaDispatcher::new(device_id);
    ...
    unsafe {
        ...
        let w_ptr = w.and_then(|s| if let NativeBuffer::Cuda(buf) = &*s.buffer { Some(buf.data.transmute::<f32>(buf.len).unwrap()) } else { None }).unwrap_or_else(|| t_buf.data.transmute::<f32>(t_buf.len).unwrap());
        let b_ptr = b.and_then(|s| if let NativeBuffer::Cuda(buf) = &*s.buffer { Some(buf.data.transmute::<f32>(buf.len).unwrap()) } else { None }).unwrap_or_else(|| t_buf.data.transmute::<f32>(t_buf.len).unwrap());
        let rm_ptr = rm.and_then(|s| if let NativeBuffer::Cuda(buf) = &*s.buffer { Some(buf.data.transmute::<f32>(buf.len).unwrap()) } else { None }).unwrap_or_else(|| t_buf.data.transmute::<f32>(t_buf.len).unwrap());
        let rv_ptr = rv.and_then(|s| if let NativeBuffer::Cuda(buf) = &*s.buffer { Some(buf.data.transmute::<f32>(buf.len).unwrap()) } else { None }).unwrap_or_else(|| t_buf.data.transmute::<f32>(t_buf.len).unwrap());
