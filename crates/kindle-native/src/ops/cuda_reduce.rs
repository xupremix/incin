...
fn ensure_reduce_loaded(device_id: usize) -> Result<()> {
    if crate::gpu::cuda_cache::get_module(device_id, "reduce").is_none() {
...
pub fn launch_reduce_op(
    ...
    if let NativeBuffer::Cuda(b) = &*storage.buffer {
        ...
        let out_strides_h: Vec<i32> =
            crate::stride::contiguous_strides(&keepdim_shape).iter().map(|&x| x as i32).collect();
        ...
        let out_strides_final = crate::stride::contiguous_strides(&out_shape);
        let keepdim_storage = NativeStorage {
            buffer: Arc::new(NativeBuffer::Cuda(out_b)),
            shape: keepdim_shape.clone(),
            strides: crate::stride::contiguous_strides(&keepdim_shape),
            offset: 0,
...
pub fn launch_reduce_with_indices_op(
    ...
    if let NativeBuffer::Cuda(b) = &*storage.buffer {
        ...
        let in_strides_h: Vec<i32> = storage.strides.iter().map(|&x| x as i32).collect();
        let out_strides_h: Vec<i32> =
            crate::stride::contiguous_strides(&out_shape).iter().map(|&x| x as i32).collect();
        ...
        let val_storage = NativeStorage {
            buffer: Arc::new(NativeBuffer::Cuda(val_b)),
            shape: out_shape.clone(),
            strides: out_strides.clone(),
            offset: 0,
            id: crate::storage::TensorId::next(),
        };
        let idx_storage = NativeStorage {
            buffer: Arc::new(NativeBuffer::Cuda(idx_b)),
            shape: out_shape,
            strides: out_strides,
            offset: 0,
            id: crate::storage::TensorId::next(),
