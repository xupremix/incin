use crate::storage::{NativeBuffer, NativeCudaBuffer, NativeStorage, TensorId};
use alloc::sync::Arc;
...
fn ensure_concat_loaded(device_id: usize) -> Result<()> {
    if crate::gpu::cuda_cache::get_module(device_id, "concat").is_none() {
...
#[cfg(feature = "cuda")]
pub fn launch_concat(tensors: &[&NativeStorage], dim: usize) -> Result<NativeStorage> {
    if tensors.is_empty() {
    ...
    let strides = crate::stride::contiguous_strides(&out_shape);
    Ok(NativeStorage {
