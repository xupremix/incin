...
#[cfg(feature = "cuda")]
pub fn launch_unary_op(op_name: &str, op_expr: &str, t: &NativeStorage) -> Result<NativeStorage> {
    if let NativeBuffer::Cuda(b) = &*t.buffer {
        ...
        if crate::gpu::cuda_cache::get_module(device_id, &kernel_name).is_none() {
            let kernel_src = ELEMENTWISE_UNARY_TEMPLATE.replace("{OP_NAME}", op_name).replace("{OP}", op_expr);
        ...
        unsafe {
            ...
            let out_slice_u8: &mut cudarc::driver::CudaSlice<u8> = Arc::get_mut(&mut out_data_arc).unwrap();
...
#[cfg(feature = "cuda")]
pub fn launch_binary_op(op_name: &str, op_expr: &str, lhs: &NativeStorage, rhs: &NativeStorage, out_shape: &[usize]) -> Result<NativeStorage> {
    if let (NativeBuffer::Cuda(lhs_b), NativeBuffer::Cuda(rhs_b)) = (&*lhs.buffer, &*rhs.buffer) {
        ...
        if crate::gpu::cuda_cache::get_module(device_id, &kernel_name).is_none() {
            let kernel_src = ELEMENTWISE_BINARY_TEMPLATE.replace("{OP_NAME}", op_name).replace("{OP}", op_expr);
            dispatcher.compile_and_load_kernel(&kernel_name, &kernel_src, &kernel_name)?;
        }
