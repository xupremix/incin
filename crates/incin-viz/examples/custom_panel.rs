...
impl incin_viz::transport_reader::TransportReader for DummyTransport {
    /// Poll new events.
    fn poll_new_events(&mut self) -> incin_viz::err::Result<Vec<Event>> {
        self.last_val += 1.0;
...
async fn main() -> anyhow::Result<()> {
    ...
    let result = incin_viz::app::run(app, terminal, keymap).await;
