//! Read-side counterpart to `incin-telemetry`'s `Transport` trait: a
...
/// `incin-telemetry::transport::file::FileTransport`. Reuses the same
...
mod tests {
    ...
    fn unique_test_path(label: &str) -> std::path::PathBuf {
        std::env::temp_dir().join(format!(
            "incin-viz-transport-reader-test-{label}-{}.jsonl",
            uuid::Uuid::now_v7()
        ))
    }
    ...
    fn scalar_event_json(step: usize, name: &str, value: f64) -> String {
        let event = Event::Scalar(incin_telemetry::events::ScalarEvent {
