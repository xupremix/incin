# Incin — Ideas: Easing the PyTorch → Incin Switch

Running list of concrete UX ideas aimed at one goal: make it as painless as
possible for a working PyTorch developer to pick up Incin. Each entry is
...
- **`print(model)` / `model.summary()` equivalent.** — ✅ IMPLEMENTED (2026-07-23). `NamedLayers::summary(&self)` and `format_layer_summary` in `incin-core/src/nn/module.rs` output formatted architecture tables.
...
- **`from_pretrained("org/model")` one-liner** — ✅ IMPLEMENTED (2026-07-23). Added `incin::hub::from_pretrained(repo_id, filename, device)`.
...
- **Compile-time `no_grad`, not a runtime context manager.** `Tensor<S, B, K,
  G>`'s fourth type parameter (`Grad`/`NoGrad`, `incin-core/src/tensor/grad.rs`)
...
- **`DataLoader` already implements `Iterator`.** Confirmed:
  `incin-data/src/loader.rs:68`'s `fn next(&mut self)`. `for batch in
...
- **A HuggingFace-Hub-style downloader already exists and is undocumented
  at the top level.** `incin-data::hub` (`HubApi`, `HubRepo::get`,
  `download(repo_id, filename)`) is already re-exported as `incin::hub`
  (`incin/src/lib.rs`'s `pub mod hub`). Combined with the just-added
...
- **Compile-time shape safety itself is the actual pitch**, and the crate's
  own doc already leads with it (`incin/src/lib.rs`'s "Key Features") — the
...
- **`getrandom`/WASM entropy source** (found 2026-07-22, during Phase 0
  NEON/WASM SIMD verification). `incin-backends` cannot currently compile
...
- **Native `.gguf` Model Export (`incin::io::export_gguf`)**.
  - *Goal*: Allow developers to train or fine-tune models in Rust using `incin` and export them directly to `.gguf` v3 format for zero-Python deployment to `ollama`, `llama.cpp`, `LM Studio`, and edge runtimes.
...
- **Apple MLX Ecosystem Interoperability (`incin::io::export_mlx`)**.
...
- **Developer UX Proposals & Utilities**:
  - **`cargo incin` Transparent Cargo Command Interceptor**:
    - ✅ IMPLEMENTED (2026-07-23). `cargo-incin` CLI binary (`crates/incin/src/bin/cargo-incin.rs`) seamlessly intercepts `cargo check`, `cargo build`, `cargo test`, `cargo run`, and `cargo clippy`.
    ...
    - Supported flags:
      - `--explain`: Appends contextual Incin rule explanations (`MatMul`, `Concat`, `Conv2d`).
      - `--raw`: Passes raw compiler diagnostics without translation.
      - `cargo incin inspect <file>`: Inspects `.safetensors`, `.gguf`, or `.onnx` model files from the command line.
  - **Model File Inspector (`incin inspect <file>`)**: Diagnostic helper / CLI tool (`incin::io::inspect(path)`) that prints a formatted tree view of tensor names, shapes, data types, quantization formats (e.g. `W4A16 (Q4_K_M)`), and memory footprint for `.safetensors`, `.gguf`, and `.onnx` files without loading full weights into GPU/RAM.
  - **`incin diff_weights(model_a, model_b)`**: Diagnostic utility for measuring weight drift, parameter divergence, or quantization error metrics ($MSE$, cosine similarity) between unquantized float models and exported/quantized checkpoints.
