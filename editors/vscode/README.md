# Kindle Shape Diagnostics (VS Code)

Humanizes Kindle's compile-time shape errors and adds real tensor-shape inlay
...
at `kindle-lsp`, a thin proxy that spawns the real rust-analyzer and rewrites
two message kinds through the `kindle-diagnostics` crate before they reach
you:
...
real `kindle-lsp`/rust-analyzer session, so it can't produce this screenshot
...
- `kindle-lsp` on your `PATH` (`cargo install --path crates/kindle-lsp --bin
  ...
  `PATH`), or set `kindle.lspPath` to its absolute path.
- The standard `rust-lang.rust-analyzer` extension installed and enabled —
  this extension configures it, it does not replace it.

## How it works

On activation (only in workspaces whose `Cargo.toml` mentions `kindle`), this
extension sets:
- `rust-analyzer.server.path` → your configured `kindle.lspPath`
  (default `kindle-lsp`, resolved via `PATH`).
- `rust-analyzer.server.extraEnv` → merges in `KINDLE_LSP_HINTS` /
  `KINDLE_LSP_SHORTEN_BACKEND` so the **Kindle: Toggle Shape Hints** command
...
diagnostics and hints (kindle-lsp's shipped defaults: hints on, backend tail
...
|---|---|---|
| `kindle.lspPath` | `"kindle-lsp"` | Path to the kindle-lsp binary. |
| `kindle.shortenBackend` | `false` | Drop the backend/dtype/grad tail from inlay hints. |

## Commands

- **Kindle: Toggle Shape Hints** — flips inlay-hint rewriting and restarts
...
containing a `Cargo.toml` that mentions `kindle`, and asserts the extension
activates and correctly rewrites `rust-analyzer.server.path`/`extraEnv`, and
that **Kindle: Toggle Shape Hints** flips the hints env var. It installs
...
Code profile. It does **not** spin up a real `kindle-lsp`/rust-analyzer
...
This produces `kindle-lsp-vscode-<version>.vsix` in this directory. Install it
into VS Code with either:

```bash
code --install-extension kindle-lsp-vscode-0.1.0.vsix
```
...
other extension (Extensions view → Kindle Shape Diagnostics → Uninstall).
