...
DEVIATIONS = {
 "EXE-008": [
   ...
   "No CUDA device is available in this environment. The CUDA executor and its migrated allocation paths are verified by all-target compilation under the cuda feature; runtime CUDA evidence remains a hardware-CI obligation, as it was for EXE-005.",
 ],
 "EXE-007": [
 ...
 "EXE-005": [
   ...
   "No CUDA device or driver is available in this environment. The generated CUDA execution probe is compiled under the cuda feature and ignored by default with an explicit hardware requirement; CPU and WGPU registrations execute here, while scheduled CUDA runtime evidence remains a hardware-CI obligation.",
 ],
 "EXE-004": [
   ...
   "CUDA buffers are represented as CudaSlice<u8>, so the Rust allocation type proves only byte alignment. CUDA TensorMeta records Alignment::BYTE instead of asserting an undocumented stronger device-pointer guarantee; a future allocator API may safely raise it.",
