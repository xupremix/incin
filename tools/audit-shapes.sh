...
def structural_rank_audit():
    """Return the remaining semantic rank generators in the source tree.

    Exact shapes use recursive `DimCons` operations. A rank generator in this
    tree would therefore be a regression, not a useful coverage measurement.
    """
    source_roots = [root / "crates/incin-core/src/shapes"]
    terms = ("rank_sweep", "generate_shape_ops", "MAX_RANK", "LEGACY_TUPLE_MAX_RANK")
    return [
...
w("| Source area | Remaining generators |\n|---|---:|\n")
w(f"| `crates/incin-core/src/shapes` | {len(structural_rank_audit())} |\n")
for path in structural_rank_audit():
    w(f"\nUnexpected generator: `{path}`\n")
PY
}
