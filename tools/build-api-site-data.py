...
def main() -> int:
    ...
    # Worked examples, generated by tools/build-api-examples.py and kept only
    # where they compile and run. Attached to the operation rather than pooled
    # at the end of a section, so an example sits with the thing it shows.
    examples_file = ROOT / "docs/api-examples.json"
    if examples_file.exists():
        examples = json.loads(examples_file.read_text(encoding="utf-8"))["examples"]
        for op in operations:
            example = examples.get(op["name"])
            if example:
                op["example"] = example
