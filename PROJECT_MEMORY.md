# PROJECT MEMORY: Incin Deep Learning Framework
...
`incin` is a high-performance, strongly-typed deep learning framework written in Rust. It enforces shape safety at compile time using `typenum` and symbolic dimensions (`s![...]`), while backing computations with native high-performance hardware execution across CPU, CUDA, and WebGPU.

### Repository Crate Structure
```
incin/
├── crates/
│   ├── incin-core/           # Core traits (Backend, Tensor, Shape, DType, Autograd Tape)
│   ├── incin-backends/       # Hardware backends (CPU, CUDA, WGPU) and kernel dispatches
│   ├── incin-macros/         # Proc macros (s![], idx![], #[module], import_model!)
│   ├── incin-telemetry/      # Real-time event logging and execution tracing
│   ├── incin-data/           # DataLoader and Dataset utilities
│   └── incin-viz/            # TUI & visualization dashboard (ratatui)
...
## 2. Core Backend Architecture: `IncinBackend<T, D>`

As of 0.2.0, `IncinBackend<T, D>` maps static device markers to their concrete
backends and maps `Dyn` to an enum-backed runtime dispatcher. Tensor device
metadata comes only from the backend:

```rust
pub type IncinBackend<T, D> = <D as BackendFor<T>>::Backend;
...
`IncinBackend<T, Dyn>` uses `DispatchBackend<T, Dyn>` and chooses CPU, WGPU,
...
Incin features compile-time type-checked shape bounds and multidimensional range slicing:
...
`incin-backends/src/dtype_policy.rs` is the authoritative capability seam.
...
- **2026-07-21 full-codebase audit**: reviewed every crate for correctness,
  ...
  `pub` API leakage in `cuda`/`cpu` backends + a new `incin-core` `TRACING_GRAPH`
  leak. Renamed `dev/refactor` → `main`; fixed a broken CI feature flag
  (`incin-backends/native` didn't exist); linked `origin` to
  `github.com/xupremix/incin` (pushed as a new branch, existing unrelated
...
- **Crate Consolidation**: Deleted `incin-native` & `incin-wgpu`, moving CPU, CUDA, and WGPU implementations under `incin-backends`.
...
- **Run CUDA Tests**: `cargo test -p incin-backends --features cuda`
...
- [x] Unify CPU, CUDA, and WGPU backends under `incin-backends`.
...
- [ ] Refactor `incin-backends` to export `IncinBackend<T, D>` with `BackendDevice<T>` trait.
