...
- **WGPU Autograd:** Implemented backward passes for `gelu`, `elu`, and `mish`
  activations in `WgpuBackend`, including WGSL gradient kernels (`gelu_grad`,
...
- **Cross-Backend Parity Tests:** New `crates/kindle-backends/tests/gradient_parity.rs`
  test suite verifies numeric agreement (≤ 1e-4) between `CpuBackend` and
  `WgpuBackend` for elementwise add, matmul, layer_norm, softmax, and
  cross_entropy_loss forward+backward passes.
- **`KindleDType::element_size()`:** New method returns the byte width of each
...
- **Safety:** `to_scalar` and `to_vec1` now validate the raw byte slice length
  against `KindleDType::element_size()` before interpreting bytes, preventing
...
- **Test Isolation:** All integration tests in `crates/kindle/tests/` now
  explicitly target `CpuBackend<f32, Cpu>` rather than `DefaultBackend`,
...
### Changed
- **`DefaultBackend`:** Always resolves to `CpuBackend<f32, Cpu>` regardless of
